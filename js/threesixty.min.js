...
!function (a) {
    ...
    a.ThreeSixty = function (b, c) {
        ...
        e.$el = a(b), e.el = b, e.$el.data("ThreeSixty", e), e.init = function () {
            ...
            }), d.styles && e.$el.css(d.styles), e.responsive(), e.$el.find(d.progress).css({marginTop: d.height / 2 + "rem"}), e.$el.find(d.progress).fadeIn("slow"), e.$el.find(d.imgList).hide()
            ...
            d.loadedImages += 1, a(d.progress + " span").text(Math.floor(d.loadedImages / d.totalFrames * 100) + "%"), d.loadedImages >= d.totalFrames ? (d.disableSpin && f[0].removeClass("previous-image").addClass("current-image"), a(d.progress).fadeOut("slow", function () {
                ...
                //扩展部分功能：图片完全加载后，注册扭转感应事件
                /*try {
                    var gammaText = "";
                    var currentGamma=0;//存放扭转值
                    window.addEventListener("deviceorientation", orientationHandler, false);
                    function orientationHandler(event) {
                        gammaText = "";
                        if(currentGamma==0){
                            e.stop();
                            currentGamma=Math.round(event.gamma);
                        }else{
                            if(Math.round(event.gamma)>currentGamma){
                                //设备逆时针扭转：右偏
                                e.previousOne();
                                currentGamma=Math.round(event.gamma);
                            }else if(Math.round(event.gamma)<currentGamma){
                                //设备顺时针扭转：左偏
                                e.nextOne();
                                currentGamma=Math.round(event.gamma);
                            }else{
                        ...
                        /!*gammaText += "左右旋转： " + Math.round(event.alpha) + "度<p>";
                         gammaText += "前后旋转： " + Math.round(event.beta) + "度<p>";
                         gammaText += "扭转设备： " + Math.round(event.gamma) + "度<p>";
                         arrow.innerHTML = gammaText;*!/
                    }
                }catch (excep) {
                    $("#arrow").html(excep.message)
                }*/
            })) : e.loadImages();
            ...
            if (d.navigation && !d.navigation_init) {
                ...
                b.append(f), b.append(g), b.append(c), e.$el.prepend(b),
            ...
            a && a.preventDefault(), d.endFrame += 5, e.refresh()
        }, e.previousOne = function (a) {
                a && a.preventDefault(), d.endFrame -= 1, e.refresh()
        }, e.nextOne = function (a) {
                a && a.preventDefault(), d.endFrame += 1, e.refresh()
        },e.play = function (a, b) {
            ...
            if (d.imgDesc!=null){
                $(d.imgDesc).text(d.imgDescList[e.getNormalizedCurrentFrame()]);
            }
